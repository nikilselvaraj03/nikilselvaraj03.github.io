...
export const NavBar = () => {
  ...
  const [profiledp, setProfile] = useState(profilePhoto);
  ...
  return (
    <>
      <nav className='navHeader'>
          <span className='title' onClick={()=> window.scrollTo(0, 0)}><Link to="/"> NIKIL SELVARAJ</Link></span>

          <ul className={close && 'show'}>
         ...
         <span className={'hamburger_menu ' + (close && 'close')} onClick={()=>{setClose(!close)}}>
          <span></span>
          <span></span>
          <span></span>
         </span>
      ...
      <div className='tagLineWrapper' ref={particlesContainer} id="particles-js">
        <span className='profile_background'>
          <img className={`fade-in-out ${isTransitioning ? 'fade-out' : ''}`} key={profiledp} src={profiledp} style={{ height: '20rem', width: '20rem', position: 'relative', top: '-5rem' }} alt='Display Picture for Nikil'></img>
        </span>
        <span className='tagline'>
          ...
          <span className='next'>About me <span className='btn'><img className='linkToLearnMore' src={learnMore} alt='linkToLearnMore' onClick={() => scrollto('about')}></img></span></span>
        </span>
